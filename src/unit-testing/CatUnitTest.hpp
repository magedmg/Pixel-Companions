...
class CatUnitTest {
    ...
    private:
        void testCatConstructor() {
            ...
            if (cat.levelUpInterval != 10 || cat.position.x != 250 || cat.position.y != 480 ||
            cat.isRunning != false || cat.isDead != false) {
                std::cout << "Cat Test 1 failed (constructor initialisations)" << std::endl;
            }
            else {
                std::cout << "Cat Test 1 succeeded (constructor initialisations)" << std::endl;
            }
        }
        ...
        void testCatUpdate() {
            ...
            if (cat.isRunning != true) {
                ...
                std::cout << "Cat Test 2 succeeded (cat update, running check)" << std::endl;
            }

            cat.Update();
            // tests if cat position is different to initial position
            if (cat.getRect().x == 250 || cat.getRect().y == 480) {
                std::cout << "Cat Test 3 failed (cat update, position check)" << std::endl;
            }
            else {
                std::cout << "Cat Test 3 succeeded (cat update, position check)" << std::endl;
            }
        }
